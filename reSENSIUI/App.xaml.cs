...
namespace reSENSIUI
{
	public partial class App : DSBaseApplication
	{
		...
		{
			this._args = e;
			Tracer.PRODUCT_SHORT_NAME = "reSENSI";
		...
		private async Task InitTypesForPrism()
		{
			...
			if (!reSENSIUI.Services.HttpClientService.InitializationIsFailedStatic)
			{
		...
		public async Task InitAfterShowWindow()
		{
			...
			if (RegistryHelper.GetValue("Config", "ShowReleaseNotes", 0, false) == 1)
			{
				...
				if (RegistryHelper.GetValue("Config", "ShowReleasePageOnStartup", 1, false) == 1)
				{
					try
					{
						Process.Start(new ProcessStartInfo(string.Format("https://www.reSENSI.com/releases/release-{0}.{1}.{2}", 6, 7, 0))
						{
		...
		{
			return new Uri("/reSENSIResources;component/Images/_ImageCatalog_X3.xaml", UriKind.Relative);
		}
		...
		{
			return new reSENSIETWTracerWrapper();
		}

		private void InitViewModelLocator()
		{
			this._viewModelTypes = this.GetTypesInNamespace(Assembly.GetExecutingAssembly(), "reSENSIUI.ViewModels");
		...
		{
			Process.Start(new ProcessStartInfo("https://www.reSENSI.com/community/")
			{
				UseShellExecute = true
			});
		}
		...
		private void UpdateDefaultStateOfRegistry()
		{
			...
			if (RegistryHelper.GetValue("Config", "Autorun", num, false) == num)
			{
				...
				Registry.SetValue("HKEY_CURRENT_USER\\Software\\Microsoft\\Windows\\CurrentVersion\\Run", "reSENSI Engine", "\"" + Path.GetDirectoryName(System.Windows.Forms.Application.ExecutablePath) + "\\reSENSIEngine.exe\"");
			}
		...
		public const string ANDROID_JUNIOR_URL = "https://play.google.com/store/apps/details?id=com.discsoft.reSENSI";
