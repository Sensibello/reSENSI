...
using reSENSICommon.Infrastructure.UDP;
...
using reSENSIUI.DataModels.GamepadActiveProfiles;
...
namespace reSENSIUI.ViewModels
{
	public class GamepadSelectorVM : BaseServicesVM, INavigationAware, IRegionMemberLifetime
	{
		...
		private void ShowMouseSettings()
		{
			...
			reSENSIApplicationCommands.NavigateGamepadCommand.Execute(typeof(MouseSettings));
		}
		...
		private void ShowVibrationSettings(bool? val)
		{
			...
			reSENSIApplicationCommands.NavigateGamepadCommand.Execute(typeof(reSENSIUI.Views.ContentZoneGamepad.VibrationSettings));
		}
		...
		{
			reSENSIApplicationCommands.NavigateGamepadCommand.Execute(typeof(MouseMappingView));
		}
		...
		{
			DSUtils.GoUrl("https://www.reSENSI.com/macro-controller/#howto");
		}
		...
		public SlotInfo CurrentSlotInfo
		{
			...
			set
			{
				if (!value.IsAvailable)
				{
					...
					reSENSIApplicationCommands.NavigateContentCommand.Execute(dictionary);
					return;
				}
