...
namespace reSENSIUI.ViewModels.Preferences
{
	public class PreferencesExternalDevicesVM : PreferencesBaseVM
	{
		...
		public bool IsExistESP32TargetRegValues(string bluetoothMacAddress)
		{
			...
			RegistryKey registryKey2 = ((registryKey != null) ? registryKey.OpenSubKey("SOFTWARE\\Disc Soft\\reSENSI\\Bluetooth\\Keys\\", false) : null);
		...
		private async Task<bool> CleanBluetoothSettings()
		{
			...
			{
				TaskAwaiter<int> taskAwaiter = adminOperations.RemoveBluetoothSettingsForreSENSI().GetAwaiter();
				if (!taskAwaiter.IsCompleted)
				{
					await taskAwaiter;
