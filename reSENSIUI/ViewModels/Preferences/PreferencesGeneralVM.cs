...
using reSENSIUI.ViewModels.Preferences.Base;
...
namespace reSENSIUI.ViewModels.Preferences
{
	public class PreferencesGeneralVM : PreferencesBaseVM
	{
		...
		public string PrivacyUrl
		{
			get
			{
				return "https://www.reSENSI.com/privacy";
			}
		}
		...
		private void StartStop()
		{
			if (this.IsStandartLogging)
			{
				...
				{
					new DiscSoftreSENSILogman().StartStop(this.IsServiceLogging, false, !this.IsGuiLogging);
				}
			...
			{
				new DiscSoftreSENSILogman().StartStop(false, this.IsBootLogging, true);
			}
		...
		private void StartProcess()
		{
			try
			{
				...
				else
				{
					...
					{
						Arguments = "/C explorer.exe " + Path.GetDirectoryName(System.Windows.Forms.Application.ExecutablePath) + "\\reSENSI.exe",
		...
		{
			if (!this.IsGUILogger && DTMessageBox.Show(string.Format(DTLocalization.GetString(this.IsGUILogger ? 12368 : 12367), "reSENSI"), MessageBoxButton.YesNo, MessageBoxImage.Question, null, false, MessageBoxResult.None) != MessageBoxResult.Yes)
			{
				return false;
			}
			if (this.IsGUILogger)
			{
				string text = string.Format(DTLocalization.GetString(12368), "reSENSI");
				string text2 = string.Format(DTLocalization.GetString(11814), "reSENSI");
		...
		{
			this.IsServiceLoggingEnabled = new DiscSoftreSENSILogman().LogmanTracers.Where((LogmanListener x) => !x.IsBootLogging).Any((LogmanListener x) => x.GetIsLogging());
			this.IsBootLoggingEnabled = new DiscSoftreSENSILogman().LogmanTracers.Where((LogmanListener x) => x.IsBootLogging).Any((LogmanListener x) => x.GetIsLogging());
		}
