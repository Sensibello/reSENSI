		...
		protected virtual async void ActivateMobileFeature()
		{
			if (this.IsLicenseNotActivated || (this.IsLicenseTrial && !this.IsFeatureHasTrialDays))
			{
				if (this.IsLicenseNeedBuy)
				{
					...
					{
						DSUtils.GoUrl(string.Format("https://www.daemon-tools.cc/cart/set_upgrade?config[type]=features&config[serial]={0}&config[features][]={1}&utm_campaign=reSENSI2&utm_source=app&utm_medium=feature", (this.LicenseInfo != null) ? this.LicenseInfo.GetValueOrDefault().Serial : null, "mobile-controller"));
					}
					else
					{
						DSUtils.GoUrl(string.Format("https://www.daemon-tools.cc/cart/buy_check?abbr=reSENSI&features[]={0}&utm_campaign=reSENSI2&utm_source=app&utm_medium=feature", "mobile-controller"));
					}
