...
namespace reSENSIUI.Controls
{
	public partial class KeyboardHookingComboBox : BaseServicesResolvingControl
	{
		...
		public IEnumerable<BasereSENSIMapping> ItemsSource
		{
			get
			{
				return (IEnumerable<BasereSENSIMapping>)base.GetValue(KeyboardHookingComboBox.ItemsSourceProperty);
			}
			set
			{
		...
		private void OnKeyboardKeyPressed(Key key)
		{
			...
			if (desiredControl.IsVisible)
			{
				ThreadHelper.ExecuteInMainDispatcher(delegate
				{
					...
					if (new ObservableCollection<BasereSENSIMapping>(this.ItemsSource).Contains(keyScanCodeV))
					{
		...
		public static readonly DependencyProperty ItemsSourceProperty = DependencyProperty.Register("ItemsSource", typeof(IEnumerable<BasereSENSIMapping>), typeof(KeyboardHookingComboBox), new PropertyMetadata(null));
