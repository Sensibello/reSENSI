...
namespace reSENSIUI.Controls.XBBindingControls.ButtonBinding
{
	public partial class SingleButtonBinding : BaseButtonBinding
	{
		...
		public ObservableCollection<BasereSENSIMapping> reSENSIMappings
		{
			get
			{
				return (ObservableCollection<BasereSENSIMapping>)base.GetValue(SingleButtonBinding.reSENSIMappingsProperty);
			}
			set
			{
				base.SetValue(SingleButtonBinding.reSENSIMappingsProperty, value);
			}
		}
		...
		{
			this.ReEvaluatereSENSIMappings();
		}
		...
		protected override void OnXBBindingChanged(DependencyPropertyChangedEventArgs e)
		{
			...
			this.GenerateDefaultCrutchBindingForRefreshingReEvaluatereSENSIMappings();
		}
		...
		private void GenerateDefaultCrutchBindingForRefreshingReEvaluatereSENSIMappings()
		{
		...
		private void ReEvaluatereSENSIMappings()
		{
			XBBinding xbbinding = base.XBBinding;
			bool flag;
			if (xbbinding == null)
			...
			List<BasereSENSIMapping> list = new List<BasereSENSIMapping>();
			if (flag17)
			{
				list.AddRange(BasereSENSIUserCommand.reSENSI_USER_COMMAND_TABLE.Where((BasereSENSIUserCommand x) => x.IsOverlayMenuCommand).ToList<BasereSENSIUserCommand>());
			}
			list.AddRange(from ksc in KeyScanCodeV2.SCAN_CODE_TABLE
				...
				select ksc);
			list.AddRange(BasereSENSIUserCommand.reSENSI_USER_COMMAND_TABLE.Where((BasereSENSIUserCommand x) => !x.IsAnyOverlayMenuCommand).ToList<BasereSENSIUserCommand>());
			if (this.IsShowVirtualGamepadButtons && flag15)
			{
				int num = list.FindLastIndex((BasereSENSIMapping item) => item.IsVirtualGyroTilt);
				BasereSENSIMapping basereSENSIMapping = list.Find((BasereSENSIMapping rm) => rm.IsVirtualGyroReset);
				if (num != -1 && basereSENSIMapping != null)
				{
					list.RemoveAll((BasereSENSIMapping rm) => rm.IsVirtualGyroReset);
					list.Insert(num + 1, basereSENSIMapping);
				}
			}
			...
			{
				list.RemoveAll(delegate(BasereSENSIMapping rm)
				{
			...
			{
				list.RemoveAll((BasereSENSIMapping rm) => rm.IsUserCommand && !rm.IsOverlayMenuCommand && !rm.IsVirtualGyroReset);
			}
			...
			{
				this.reSENSIMappings = new ObservableCollection<BasereSENSIMapping>(list);
			}
		}

		private bool IsSameMappings(List<BasereSENSIMapping> filteredreSENSIMappings)
		{
			if (this.reSENSIMappings == null || filteredreSENSIMappings.Count != this.reSENSIMappings.Count)
			{
				return false;
			}
			int count = filteredreSENSIMappings.Count;
			for (int i = 0; i < count; i++)
			{
				if (filteredreSENSIMappings[i] != this.reSENSIMappings[i])
				{
					return false;
				}
			}
		...
		public void OnKeyboardKeyPressed(Key key)
		{
			...
			ThreadHelper.ExecuteInMainDispatcher(delegate
			{
				...
				if (((xbbinding != null) ? xbbinding.CurrentActivatorXBBinding : null) != null && !string.IsNullOrWhiteSpace(keyScanCodeV.Description) && this.reSENSIMappings.Contains(keyScanCodeV))
				{
		...
		public static readonly DependencyProperty reSENSIMappingsProperty = DependencyProperty.Register("reSENSIMappings", typeof(ObservableCollection<BasereSENSIMapping>), typeof(SingleButtonBinding), new PropertyMetadata(null));
