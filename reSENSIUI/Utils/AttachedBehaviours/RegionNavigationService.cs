...
using Prism.Regions;
using reSENSIUI.Controls;
using reSENSIUI.Infrastructure;

namespace reSENSIUI.Utils.AttachedBehaviours
{
	public class RegionNavigationService
	{
		...
		private static void OnUrlClick(object sender, RoutedEventArgs e)
		{
			...
			try
			{
				...
				CompositeCommand compositeCommand = reSENSIApplicationCommands.NavigateContentCommand;
				if (navigationInfo.RegionName.Equals(RegionNames.Content))
				{
					compositeCommand = reSENSIApplicationCommands.NavigateContentCommand;
				}
				else if (navigationInfo.RegionName.Equals(RegionNames.Sidebar))
				{
					compositeCommand = reSENSIApplicationCommands.NavigateSideBarRegionCommand;
				}
				else if (navigationInfo.RegionName.Equals(RegionNames.Gamepad))
				{
					compositeCommand = reSENSIApplicationCommands.NavigateGamepadCommand;
				}
				else if (navigationInfo.RegionName.Equals(RegionNames.BindingFrame))
				{
					compositeCommand = reSENSIApplicationCommands.NavigateBindingFrameCommand;
				}
				if (type != null)
				{
