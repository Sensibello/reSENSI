...
using reSENSIUI.Utils.Converters;
...
namespace GenGamepadView.Views
{
    public class MouseMappingView : UserControl, IComponentConnector
    {
        ...
        private void InitContainerGrid()
        {
            ...
            foreach (XBBinding xBBinding in XBBindings)
            {
                if (xBBinding.IsNotEmpty)
                {
                    ...
                    string name = reSENSIUI.Utils.XBUtil.XBUtils.ConvertGamepadButtonToAnchorString(xBBinding.GamepadButton) + "Annotation";
            ...
            foreach (ControllerBinding controllerBinding in XBBindings.ControllerBindings)
            {
                ...
                try
                {
                    text = reSENSIUI.Utils.XBUtil.XBUtils.ConvertMouseButtonToAnchorString(KeyScanCodeV2.FindMouseButtonByKeyScanCode(controllerBinding.XBBinding.KeyScanCode));
                }
                catch (Exception)
                {
                    continue;
        ...
        public void InitializeComponent()
        {
            ...
            {
                _contentLoaded = true;
                Uri resourceLocator = new Uri("/reSENSI;component/controls/fastconfigrenders/mousemappingview.xaml", UriKind.Relative);
