...
namespace reSENSIUI.DataModels
{
	public class ConfigVM : ZBindableBase, IComparable
	{
		...
		public void ApplyPreset(ConfigVM preset)
		{
			if (MessageBoxWithDoNotShowLogic.Show(Application.Current.MainWindow, DTLocalization.GetString(12569), MessageBoxButton.YesNo, MessageBoxImage.Question, "ConfirmAddOrDeleteShift", MessageBoxResult.Yes, false, 0.0, null, null, null, null, null, null) == MessageBoxResult.Yes)
			{
				...
				reSENSIApplicationCommands.FillShift(configData, 1, this, shiftIndex, false, true);
				reSENSIApplicationCommands.FillPresetDirections(configData, 0, this, shiftIndex, true);
				this.ConfigData.CheckVirtualMappingsExist();
				WaitDialog.TryCloseWaitDialog();
		...
		private async void ApplyToCurrentPreset()
		{
			...
			if (result != MessageBoxResult.Cancel)
			{
				...
				else
				{
					...
					reSENSIApplicationCommands.FillShift(configData, 1, App.GameProfilesService.CurrentGame.CurrentConfig, App.GameProfilesService.RealCurrentBeingMappedBindingCollection.ShiftIndex, true, result == MessageBoxResult.Yes);
					reSENSIApplicationCommands.FillPresetDirections(configData, 0, App.GameProfilesService.CurrentGame.CurrentConfig, App.GameProfilesService.RealCurrentBeingMappedBindingCollection.ShiftIndex, result == MessageBoxResult.Yes);
					this.ConfigData.CheckVirtualMappingsExist();
