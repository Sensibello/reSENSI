...
namespace reSENSIUI.DataModels.GamepadActiveProfiles
{
	...
	public class GamepadProfilesCollection : ObservableDictionary<string, GamepadProfiles>
	{
		...
		public void SetGameName(string sNewName)
		{
			...
			while (crutchEnumerator.MoveNext())
			{
				try
				{
					...
					{
						keyValuePair2.Value.ConfigPath = Path.Combine(App.UserSettingsService.ConfigsFolderPath, sNewName + "\\Controller\\" + keyValuePair2.Value.ProfileName);
					}
				}
				catch (Exception)
				{
		...
		public bool RemoveConfig(ConfigVM config)
		{
