...
using reSENSIUI.Views.SecondaryWindows.CalibrateGyroscope;
...
namespace reSENSIUI.Infrastructure.Controller
{
	public abstract class BaseControllerVM : ZBindableBase, IBaseController, IEquatable<BaseControllerVM>
	{
		...
		public string ControllerFriendlyName
		{
			get
			{
				if (this.CurrentController != null)
				{
					...
					{
						return string.Format(DTLocalization.GetString(11802), "https://www.reSENSI.com/#contacts");
					}
		...
		public DelegateCommand DoNotShowInreSENSICommand
		{
			get
			{
				DelegateCommand delegateCommand;
				if ((delegateCommand = this._doNotShowInreSENSI) == null)
				{
					delegateCommand = (this._doNotShowInreSENSI = new DelegateCommand(new Action(this.DoNotShowInreSENSI)));
				}
				return delegateCommand;
			}
		}

		private void DoNotShowInreSENSI()
		{
		...
		private DelegateCommand _doNotShowInreSENSI;
