...
namespace reSENSIUI.Infrastructure.KeyBindings.MacroBinding
{
	public abstract class BaseMacro : ZBindableBase
	{
		...
		{
			reSENSIApplicationCommands.CopyMacroSequence(this.MacroSequence);
		}

		private bool CopySelectedItemsCanExecute()
		{
			return reSENSIApplicationCommands.CanCopyMacroSequence(this.MacroSequence) && (this.MacroSequence.IsHoldUntilRelease || this.MacroSequence.IsMultiSelectedItems || (!(this is MacroGamepadBinding) && !(this is MacroKeyBinding)));
		}
		...
		public Visibility CopyCommandVisibility
		{
			get
			{
				...
				if (reSENSIApplicationCommands.CanCopyMacroSequence(this.MacroSequence) && (this.MacroSequence.IsMultiSelectedItems || (!(this is MacroGamepadBinding) && !(this is MacroKeyBinding))))
				{
					return Visibility.Visible;
				}
		...
		public DelegateCommand PasteCommand
		{
			get
			{
				...
				{
					delegateCommand = (this._pasteCommand = new DelegateCommand(new Action(this.PasteSelectedItems), () => reSENSIApplicationCommands.CanPasteMacroSequence()));
				}
				return delegateCommand;
			}
		}
		...
		{
			reSENSIApplicationCommands.PasteMacroSequence(this.MacroSequence, this.MacroSequence.IndexOf(this) + 1);
		}
		...
		public DelegateCommand ReplaceCommand
		{
			get
			{
				...
				if ((delegateCommand = this._replaceCommand) == null)
				{
					...
					}, () => reSENSIApplicationCommands.CanPasteMacroSequence()));
				}
				return delegateCommand;
			}
		}
		...
		private void ReplaceSelectedItems()
		{
			if (this.MacroSequence != null)
			{
				reSENSIApplicationCommands.ReplaceMacroSequence(this.MacroSequence, this.MacroSequence.IndexOf(this) + 1);
			}
		}
