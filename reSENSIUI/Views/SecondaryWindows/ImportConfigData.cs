...
using reSENSICommon.Infrastructure.XML;
		...
		public ImportConfigData(string configPath, GameVM currentGame, bool isCloning)
		{
			...
			try
			{
				if (currentGame != null)
				{
					...
					{
						this.ConfigName = Path.GetFileNameWithoutExtension(DSUtils.GetUniqueFileName(text, "Config.reSENSI"));
					}
				}
				else if (!string.IsNullOrEmpty(configPath))
				{
					...
					else if (XBUtils.CheckProfileFirstChar(configPath, '<'))
					{
						...
						if (num == 1.1)
						{
							new reSENSIconfigWrapper_1_1().GetNodeValue(configPath, "app_name", ref xmlAppNameValue);
						}
						else if (num == 1.2)
						{
							new reSENSIconfigWrapper_1_2().GetNodeValue(configPath, "app_name", ref xmlAppNameValue);
						}
						else if (num == 2.0)
						{
							new reSENSIconfigWrapper_2_0().GetNodeValue(configPath, "app_name", ref xmlAppNameValue);
						}
						else if (num == 2.1)
						{
							new reSENSIconfigWrapper_2_1().GetNodeValue(configPath, "app_name", ref xmlAppNameValue);
						}
