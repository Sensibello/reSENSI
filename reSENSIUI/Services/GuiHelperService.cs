...
using reSENSIEngine.Services.HttpServer.Data;
...
using reSENSIUI.Infrastructure.KeyBindings.ControllerBindings;
using reSENSIUI.Infrastructure.KeyBindings.MacroBinding;
using reSENSIUI.Infrastructure.KeyBindings.XB;
using reSENSIUI.License.Views;
using reSENSIUI.Services.HttpClient;
using reSENSIUI.Services.Interfaces;
using reSENSIUI.Utils.XBUtil;
using reSENSIUI.ViewModels;
using reSENSIUI.Views;
using reSENSIUI.Views.ContentZoneGamepad;
using reSENSIUI.Views.ContentZoneGamepad.AdvancedStick;
using reSENSIUI.Views.ContentZoneGamepad.Macro;
using reSENSIUI.Views.OverlayMenu;
using reSENSIUI.Views.SecondaryWindows;
using reSENSIUI.Views.SecondaryWindows.AddExternalDeviceWizard;
using reSENSIUI.Views.VirtualSticks;
...
namespace reSENSIUI.Services
{
	public class GuiHelperService : ZBindableBase, IGuiHelperService
	{
		...
		public async void CreateConfigExecute(GameVM game)
		{
			...
			configVM.Name = Path.GetFileNameWithoutExtension(DSUtils.GetUniqueFileName(gameFolderPath, "Config.reSENSI"));
		...
		private async Task CreateNewConfig(GameVM game, ConfigVM configVM, string gameFolderPath)
		{
			...
			if (!string.IsNullOrEmpty(configVM.Name))
			{
				...
				configVM.ConfigPath = gameFolderPath + XBUtils.NormalizeToMaxPathTrimFilename(gameFolderPath, configVM.Name.Trim(), ".reSENSI", null) + ".reSENSI";
		...
		private void BindingFrameBack(object param)
		{
			...
			{
				reSENSIApplicationCommands.NavigateGamepadCommand.Execute(typeof(OverlayMenuView));
				reSENSIApplicationCommands.NavigateBindingFrameCommand.Execute(null);
			}
			else
			{
				...
				{
					reSENSIApplicationCommands.NavigateGamepadCommand.Execute(typeof(MaskView));
					reSENSIApplicationCommands.NavigateBindingFrameCommand.Execute(null);
				}
				else
				{
					...
					if (gamepadButton != null)
					{
						if (GamepadButtonExtensions.IsAnyTriggerZone(gamepadButton.Value) || GamepadButtonExtensions.IsAnyPhysicalTrackPadPressureZone(gamepadButton.Value) || GamepadButtonExtensions.IsDS3AnalogZone(gamepadButton.Value))
						{
							...
							{
								reSENSIApplicationCommands.NavigateGamepadCommand.Execute(typeof(SVGGamepadWithAllAnnotations));
								reSENSIApplicationCommands.NavigateBindingFrameCommand.Execute(typeof(BFAdvanced));
								return;
							}
						...
						if (GamepadButtonExtensions.IsAnyStickDiagonalDirection(gamepadButton.Value) || GamepadButtonExtensions.IsAnyPhysicalTrackDiagonalDirection(gamepadButton.Value) || GamepadButtonExtensions.IsAnyStickZone(gamepadButton.Value) || GamepadButtonExtensions.IsGyroTiltZone(gamepadButton.Value) || GamepadButtonExtensions.IsMouseZone(gamepadButton.Value) || GamepadButtonExtensions.IsGyroLean(gamepadButton.Value))
						{
							if (App.RegionManager.Regions[RegionNames.Gamepad].ActiveViews.FirstOrDefault<object>() is MacroSettings)
							{
								reSENSIApplicationCommands.NavigateGamepadCommand.Execute(typeof(AdvancedStickSettings));
								return;
							}
					...
					if (flag5)
					{
						...
						{
							reSENSIApplicationCommands.NavigateGamepadCommand.Execute(typeof(KeyboardMappingView));
							return;
						}
					}
					...
					reSENSIApplicationCommands.NavigateBindingFrameCommand.Execute(param);
				}
			}
			GC.Collect();
		...
		public void ClearConfigExecute(ConfigVM config)
		{
			...
			{
				WaitDialog.ShowDialogStatic(DTLocalization.GetString(5238), null, null, false, false, null, null);
				reSENSIApplicationCommands.ClearConfig(config);
				WaitDialog.TryCloseWaitDialog();
			}
		}
		...
		public async void RenameConfigExecute(ConfigVM config)
		{
			...
			if (taskAwaiter.GetResult() != 3 && this.ShowEditWindow(config, out newName))
			{
				...
				newName = XBUtils.NormalizeToMaxPathTrimFilename(directoryName, newName.Trim(), ".reSENSI.bak", null);
				if (oldName != newName && !string.IsNullOrEmpty(newName))
				{
					...
					{
						string text = directoryName + "\\" + XBUtils.NormalizeToMaxPathTrimFilename(directoryName + "\\", newName.Trim(), ".reSENSI", null) + ".reSENSI";
		...
		private void SwitchVirtualStickSettingsViewMode()
		{
			...
			{
				reSENSIApplicationCommands.NavigateGamepadCommand.Execute(typeof(VirtualSticksSettingsView));
			}
			else
			{
				GameVM currentGame = App.GameProfilesService.CurrentGame;
		...
		private void SwitchLEDSettingsViewMode()
		{
			...
			{
				reSENSIApplicationCommands.NavigateGamepadCommand.Execute(typeof(LedSettingsView));
				return;
			}
			GameVM currentGame = App.GameProfilesService.CurrentGame;
			if (((currentGame != null) ? currentGame.CurrentConfig : null) != null)
