...
using reSENSIEngine.Services;
using reSENSIEngine.Services.HttpServer;
using reSENSIEngine.Services.Interfaces;
using reSENSIEngine.Services.UdpServer;
...
namespace reSENSIEngine
{
	public partial class Engine : DSBaseApplication
	{
		...
		protected override async void OnStartup(StartupEventArgs e)
		{
			...
			Tracer.PRODUCT_SHORT_NAME = "reSENSI engine";
			...
			else
			{
				...
				if (UtilsNative.IsWindows10OrHigher() && !XBUtils.CheckCertDll("reSENSIPolicy.dll"))
				{
					Environment.Exit(0);
				}
				base.OnStartup(e);
