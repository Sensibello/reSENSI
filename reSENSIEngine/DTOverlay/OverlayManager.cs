...
using reSENSIEngine;
using reSENSIEngine.OverlayAPI.RemapWindow;
using reSENSIEngine.Services.OverlayAPI;
using Unity;
...
namespace DTOverlay
{
	public class OverlayManager
	{
		...
		{
			string text = Path.Combine(Environment.GetFolderPath(Environment.SpecialFolder.CommonApplicationData), "Disc-Soft", "reSENSI");
			text += "\\OverlayData";
			try
			{
		...
		private void OverlayGamepadHotkey(string ID, string controllerDisplayName, string gameName, string profileName)
		{
			...
			using (IEnumerator<Wrapper<reSENSI_CONTROLLER_PROFILE_EX>> enumerator = Engine.GamepadService.ServiceProfilesCollection.Where((Wrapper<reSENSI_CONTROLLER_PROFILE_EX> x) => x.Value.ProfilesState[x.Value.GetCurrentSlot()].Enabled && x.Value.ProfilesState[x.Value.GetCurrentSlot()].VirtualType > 0U).GetEnumerator())
			{
				if (enumerator.MoveNext())
				{
					Wrapper<reSENSI_CONTROLLER_PROFILE_EX> wrapper = enumerator.Current;
					Slot currentSlot = wrapper.Value.GetCurrentSlot();
					reSENSI_GET_PROFILE_STATE_RESPONSE reSENSI_GET_PROFILE_STATE_RESPONSE = wrapper.Value.ProfilesState[currentSlot];
					string text = reSENSI_GET_PROFILE_STATE_RESPONSE.VirtualId.ToString();
					ControllerTypeEnum controllerTypeEnum = ControllerTypeExtensions.ConvertPhysicalTypeToEnum(0, reSENSI_GET_PROFILE_STATE_RESPONSE.VirtualType, 0UL);
		...
		private void OnConfigAppliedToSlot(BaseControllerVM controller, GamepadProfile gamepadProfile, Slot iSlot, reSENSI_CONTROLLER_PROFILE? profile)
		{
			...
			if (profile != null)
			{
				...
				CS$<>8__locals1.isNeedAnyFeature = reSENSI_CONTROLLER_PROFILE_Extensions.IsNeedAnyFeature(profile.Value);
			}
		...
		private void OverlaySlotChange(BaseControllerVM controller, GamepadProfile gamepadProfile, Slot iSlot, reSENSI_CONTROLLER_PROFILE? profile, bool physical)
		{
			...
			if (profile != null)
			{
				...
				CS$<>8__locals1.empty = !reSENSI_CONTROLLER_PROFILE_Extensions.IsSlotWithUserConfig(profile.Value);
		...
		private async void LoadBlackList()
		{
			...
			if ((DateTime.Now - minValue).TotalSeconds > 86000.0)
			{
				try
				{
					...
					string text = await new HttpClient().GetStringAsync("https://services.reSENSI.com/reSENSI/games/processNames");
					if (text.Length == 0)
					{
		...
		private async Task<int> ReadVirtualControllerStates()
		{
			...
			foreach (Wrapper<reSENSI_CONTROLLER_PROFILE_EX> wrapper in Engine.GamepadService.ServiceProfilesCollection.Where((Wrapper<reSENSI_CONTROLLER_PROFILE_EX> x) => x.Value.ProfilesState[x.Value.GetCurrentSlot()].Enabled && x.Value.ProfilesState[x.Value.GetCurrentSlot()].VirtualType > 0U))
			{
				Slot currentSlot = wrapper.Value.GetCurrentSlot();
				reSENSI_GET_PROFILE_STATE_RESPONSE reSENSI_GET_PROFILE_STATE_RESPONSE = wrapper.Value.ProfilesState[currentSlot];
				ushort num = wrapper.Value.ServiceProfileIds[currentSlot];
				string virtualId = reSENSI_GET_PROFILE_STATE_RESPONSE.VirtualId.ToString();
				ControllerTypeEnum controllerType = ControllerTypeExtensions.ConvertPhysicalTypeToEnum(0, reSENSI_GET_PROFILE_STATE_RESPONSE.VirtualType, 0UL);
			...
			IEnumerator<Wrapper<reSENSI_CONTROLLER_PROFILE_EX>> enumerator2 = null;
			return 1;
		}
		...
		{
			this.overlayMenuE.ExecuteOverlayMenuCommand(reSENSIOverlayMenuServiceCommand);
		}
