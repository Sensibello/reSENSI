...
namespace DTOverlay
{
	public static class OverlayUtils
	{
		...
		{
			foreach (FileInfo fileInfo in new DirectoryInfo(Path.Combine(Environment.GetFolderPath(Environment.SpecialFolder.CommonApplicationData), "Disc-Soft", "reSENSI") + "\\OverlayData\\").GetFiles(prefix + "*"))
			{
				int num = 0;
