	...
	internal class MainWindowViewModel : ZBindable
	{
		...
		public async void AdjustAgentSettings(ControllerVM controller)
		{
			...
			if (taskAwaiter.GetResult())
			{
				...
				XBUtils.RunreSENSIGui("/OpenAgentSettings");
			}
		}
		...
		public static bool GUIIsNotRunning(string mutexName)
		{
			...
			while (flag)
			{
				...
				if (!flag2)
				{
					flag = DTMessageBox.Show(string.Format(DTLocalization.GetString(4488), "reSENSI"), "reSENSI", MessageBoxButton.OKCancel) == MessageBoxResult.OK;
				}
			}
			return flag2;
		}
		...
		{
			MainWindowViewModel.SendCloseGUI("DiscSoftreSENSI_msg_uninstall");
			Thread.Sleep(1000);
