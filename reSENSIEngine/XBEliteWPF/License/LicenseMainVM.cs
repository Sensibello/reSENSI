	...
	public class LicenseMainVM
	{
		...
		private async Task SetFeaturesPrices()
		{
			try
			{
				...
				string text = await new HttpClient().GetStringAsync(string.Format("https://secure.disc-soft.com/license/features/software_abbr/reSENSI/locale/{0}?paid={1}", slocale, (licenseInfo.License == 3) ? "1" : "0"));
				if (text.Length != 0)
				{
		...
		public async void BuyFeature(string featureGuid)
		{
			...
			if (feature != null)
			{
				...
				else
				{
					...
					{
						DSUtils.GoUrl(string.Format("https://www.daemon-tools.cc/cart/set_upgrade?config[type]=features&config[serial]={0}&config[features][]={1}&utm_campaign=reSENSI2&utm_source=app&utm_medium=feature", (await LicenseApi.GetLicenseInfo()).Serial, feature.FeatureLinkName));
					}
