...
namespace XBEliteWPF.License.Licensing
{
	public static class LicenseApi
	{
		...
		{
			ServiceResponseWrapper<reSENSI_GET_VERSION_RESPONSE> serviceResponseWrapper = await LicenseApi.LicensingCommunicator.GetVersion();
		...
		public static async Task<bool> IsOfferExist()
		{
			...
			try
			{
				...
				ServiceLicensingResponseWrapper<reSENSI_LICENSE_CONTROL_RESPONSE> serviceLicensingResponseWrapper = await LicenseApi.LicensingCommunicator.LicenseControl(array, num, 2U);
				if (serviceLicensingResponseWrapper.IsResponseValid && (ulong)serviceLicensingResponseWrapper.ServiceResponse.Header.Size == (ulong)(2L + (long)Marshal.SizeOf<reSENSI_RESPONSE_HEADER>()))
				{
		...
		public static async Task ClearOffer()
		{
			try
			{
				...
				if (LicenseApi._serviceVersionEqual)
				{
					...
					uint num = (uint)array.Length;
					ServiceLicensingResponseWrapper<reSENSI_LICENSE_CONTROL_RESPONSE> serviceLicensingResponseWrapper = await LicenseApi.LicensingCommunicator.LicenseControl(array, num, 1U);
		...
		public static async Task<bool> GetLicenseInfo0(string fileName)
		{
			try
			{
				...
				ServiceLicensingResponseWrapper<reSENSI_LICENSE_CONTROL_RESPONSE> serviceLicensingResponseWrapper = await LicenseApi.LicensingCommunicator.LicenseControl(array2, num, outputBufferSize);
				if (serviceLicensingResponseWrapper.IsResponseValid && (ulong)serviceLicensingResponseWrapper.ServiceResponse.Header.Size == (ulong)outputBufferSize + (ulong)((long)Marshal.SizeOf<reSENSI_RESPONSE_HEADER>()))
				{
					return true;
				}
		...
		public static async Task<bool> Function12(string partnerId)
		{
			try
			{
				...
				uint num = (uint)array.Length;
				ServiceLicensingResponseWrapper<reSENSI_LICENSE_CONTROL_RESPONSE> serviceLicensingResponseWrapper = await LicenseApi.LicensingCommunicator.LicenseControl(array, num, 0U);
				if (serviceLicensingResponseWrapper.IsResponseValid && (ulong)serviceLicensingResponseWrapper.ServiceResponse.Header.Size == (ulong)((long)Marshal.SizeOf<reSENSI_RESPONSE_HEADER>()))
				{
					return true;
				}
