...
namespace XBEliteWPF.Infrastructure.Controller
{
	...
	public abstract class BaseControllerVM : IBaseController
	{
		public ushort[] ServiceProfileIDs
		{
			get
			{
				Wrapper<reSENSI_CONTROLLER_PROFILE_EX> wrapper = Engine.GamepadService.ServiceProfilesCollection.FindWrapperByID(this.ID);
				if (wrapper == null)
				{
					return new ushort[4];
				}
		...
		{
			reSENSI_CONTROLLER_PROFILE_EX profileEx = Engine.GamepadService.GetProfileEx(this.ID);
			if (profileEx != null)
			{
				for (int i = 0; i < 4; i++)
				{
					ServiceResponseWrapper<reSENSI_GET_PROFILE_STATE_RESPONSE> serviceResponseWrapper = await Engine.XBServiceCommunicator.GetProfileState(profileEx.ServiceProfileIds[i], false);
					if (serviceResponseWrapper.ServiceResponse.Enabled && !reSENSI_GET_PROFILE_STATE_RESPONSE_Extension.IsSlotManagerProfile(serviceResponseWrapper.ServiceResponse))
					{
						this.CurrentSlot = reSENSI_GET_PROFILE_STATE_RESPONSE_Extension.GetSlot(serviceResponseWrapper.ServiceResponse);
						break;
					}
				}
			}
		...
		public bool IsAmiiboApplied
		{
			get
			{
				...
				{
					flag = new bool?(amiibo.Any((reSENSI_CONTROLLER_PROFILE_EX.AmiiboWrapper item) => item.AmiiboLoaded));
				}
				bool? flag2 = flag;
				bool flag3 = true;
		...
		public abstract bool UpdateControllerInfo(reSENSI_CONTROLLER_INFO controllerInfo);

		public abstract void UpdateControllerState(reSENSI_GET_CONTROLLER_STATE_RESPONSE controllerState);
