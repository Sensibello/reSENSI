...
namespace XBEliteWPF.Infrastructure.ExternalDeviceRelations
{
	public class ExternalDeviceRelationsHelper : ZBindable
	{
		...
		public ExternalDeviceState GetExternalDeviceStateWithProfiles(bool isConfigExternal, VirtualGamepadType? configVirtualGamepadType, Config config, BaseControllerVM gamepad, Slot? slot, ExternalDevice externalDevice, ExternalClient externalClient, GamepadAuth gamepadAuth, reSENSI_CONTROLLER_PROFILE_EX profileEx = null)
		{
			...
			{
				reSENSI_CONTROLLER_PROFILE_EX reSENSI_CONTROLLER_PROFILE_EX = null;
				if (externalDevice != null && this.GamepadService.IsAnyServiceProfileWithExternalDevice(externalDevice.DeviceType))
				{
					reSENSI_CONTROLLER_PROFILE_EX = this.GamepadService.GetProfileEx((gamepad != null) ? gamepad.ID : null);
				}
				if (reSENSI_CONTROLLER_PROFILE_EX != null)
				{
					ExternalDeviceState externalDeviceState2 = this.GetExternalDeviceState(isConfigExternal, configVirtualGamepadType, config, gamepad, slot, externalDevice, externalClient, gamepadAuth, reSENSI_CONTROLLER_PROFILE_EX);
		...
		public ExternalDeviceState GetExternalDeviceState(bool isConfigExternal, VirtualGamepadType? configVirtualGamepadType, Config config, BaseControllerVM gamepad, Slot? slot, ExternalDevice externalDevice, ExternalClient externalClient, GamepadAuth gamepadAuth, reSENSI_CONTROLLER_PROFILE_EX profileEx = null)
		{
			...
			if (this.GamepadService.IsAnyServiceProfileWithExternalDevice(externalDevice.ExternalDeviceId) && !Convert.ToBoolean(RegistryHelper.GetValue("Config", "ExternalDeviceOverwritePrevConfig", 0, false)))
			{
				...
				if (profileEx != null && this._gameProfilesService != null && slot.Value < (ulong)((long)profileEx.Profiles.Length))
				{
					...
					if (gamepadProfile != null && this.GamepadService.GetRemapState(gamepad.ID) == 1 && config != null && config.ConfigPath != null && !string.IsNullOrEmpty((config != null) ? config.ConfigPath : null) && gamepadProfile.ConfigPath != ((config != null) ? config.ConfigPath : null) && reSENSI_CONTROLLER_PROFILE_Extensions.GetExternalDeviceId(profileEx.Profiles[slot.Value]) == externalDevice.ExternalDeviceId)
					{
						return 5;
					}
				}
		...
		public ExternalState GetExternalState(bool isConfigExternal, VirtualGamepadType? configVirtualGamepadType, Config config, BaseControllerVM gamepad, Slot? slot, ExternalDevice externalDevice, ExternalClient externalClient, GamepadAuth gamepadAuth)
		{
			...
			ExternalDeviceState externalDeviceState = this.GetExternalDeviceState(isConfigExternal, configVirtualGamepadType, config, gamepad, slot, externalDevice, externalClient, gamepadAuth, reSENSI_CONTROLLER_PROFILE_EX);
			if (externalDeviceState != 2)
			{
			...
			if (gamepad != null && this.GamepadService.GetRemapState(gamepad.ID) == 1 && reSENSI_CONTROLLER_PROFILE_EX != null && config != null && config.ConfigPath != null && !string.IsNullOrEmpty((config != null) ? config.ConfigPath : null))
			{
				GamepadProfiles gamepadActiveProfiles = this.GamepadService.GetGamepadActiveProfiles(gamepad);
				...
				if (slotProfilesDictionary != null)
				{
					...
					{
						if (keyValuePair.Value.ConfigPath == ((config != null) ? config.ConfigPath : null) && reSENSI_CONTROLLER_PROFILE_EX.Enabled[slot.Value])
						{
							reSENSI_GET_PROFILE_STATE_RESPONSE reSENSI_GET_PROFILE_STATE_RESPONSE = reSENSI_CONTROLLER_PROFILE_EX.ProfilesState[slot.Value];
							if (reSENSI_GET_PROFILE_STATE_RESPONSE.RemoteConnected)
							{
								return 9;
							}
							if (reSENSI_CONTROLLER_PROFILE_EX.ExternalClientDisconnected[slot.Value])
							{
								return 7;
							}
							if (reSENSI_GET_PROFILE_STATE_RESPONSE.BluetoothAuthenticationMethod != 0)
							{
								return 8;
							}
							return 6;
		...
		{
			if (!this.GamepadService.ServiceProfilesCollection.Any((Wrapper<reSENSI_CONTROLLER_PROFILE_EX> x) => x.Value.IsAnyExternalDevicePresent()))
			{
			...
			bool flag2 = false;
			bool flag3 = false;
			List<Wrapper<reSENSI_CONTROLLER_PROFILE_EX>> serviceProfileWithExternalBluetoothAdapter = this.GamepadService.GetServiceProfileWithExternalBluetoothAdapter();
			List<Wrapper<reSENSI_CONTROLLER_PROFILE_EX>> serviceProfileWithExternalBluetooth = this.GamepadService.GetServiceProfileWithExternalBluetooth();
			List<Wrapper<reSENSI_CONTROLLER_PROFILE_EX>> serviceProfileWithExternalSerialPort = this.GamepadService.GetServiceProfileWithExternalSerialPort();
			if (serviceProfileWithExternalBluetoothAdapter.Count > 0 && !BluetoothUtils.CanUseBluetoothInreSENSI)
			{
			...
			{
				foreach (Wrapper<reSENSI_CONTROLLER_PROFILE_EX> wrapper in serviceProfileWithExternalBluetooth)
				{
					int num = 0;
					reSENSI_CONTROLLER_PROFILE[] array = wrapper.Value.Profiles;
					for (int i = 0; i < array.Length; i++)
					{
						reSENSI_CONTROLLER_PROFILE bluetoothProfile = array[i];
						if (num >= wrapper.Value.Enabled.Length || num >= wrapper.Value.ProfilesState.Length)
						{
							break;
						}
						if (reSENSI_CONTROLLER_PROFILE_Extensions.IsExternalDeviceWithBluetoothPresent(bluetoothProfile) && wrapper.Value.Enabled[num])
						{
			...
			if (serviceProfileWithExternalSerialPort.Count > 0)
			{
				...
				foreach (Wrapper<reSENSI_CONTROLLER_PROFILE_EX> wrapper2 in serviceProfileWithExternalSerialPort)
				{
					int num2 = 0;
					reSENSI_CONTROLLER_PROFILE[] array = wrapper2.Value.Profiles;
					for (int i = 0; i < array.Length; i++)
					{
						reSENSI_CONTROLLER_PROFILE gimxProfile = array[i];
						if (num2 >= wrapper2.Value.Enabled.Length || num2 >= wrapper2.Value.ProfilesState.Length)
						{
							break;
						}
						if (reSENSI_CONTROLLER_PROFILE_Extensions.IsExternalDeviceWithSerialPortPresent(gimxProfile) && wrapper2.Value.Enabled[num2])
						{
			...
			case 2:
				if (this.CurrentExternalDevice != null)
				{
					...
					else
					{
						...
						if (BluetoothUtils.NeedInitForreSENSI())
						{
		...
		public bool CheckInUseWithOtherDevices(ExternalDeviceType externalDeviceType, string externalDeviceId, ulong[] currentControllerIds)
		{
			...
			List<Wrapper<reSENSI_CONTROLLER_PROFILE_EX>> serviceProfileWithExternalDevice = this.GamepadService.GetServiceProfileWithExternalDevice(externalDeviceType);
			Func<reSENSI_CONTROLLER_PROFILE, bool> <>9__2;
			Func<ulong, bool> <>9__4;
			return serviceProfileWithExternalDevice != null && serviceProfileWithExternalDevice.Where((Wrapper<reSENSI_CONTROLLER_PROFILE_EX> profileEx) => profileEx.Value.IsExternalDevicePresent(externalDeviceType)).Any(delegate(Wrapper<reSENSI_CONTROLLER_PROFILE_EX> profileEx)
			{
				IEnumerable<reSENSI_CONTROLLER_PROFILE> profiles = profileEx.Value.Profiles;
				Func<reSENSI_CONTROLLER_PROFILE, bool> func;
				if ((func = <>9__2) == null)
				{
					func = (<>9__2 = (reSENSI_CONTROLLER_PROFILE profile) => reSENSI_CONTROLLER_PROFILE_Extensions.IsExternalDeviceEqual(profile, externalDeviceId));
				}
				IEnumerable<ulong> enumerable = profiles.Where(func).SelectMany((reSENSI_CONTROLLER_PROFILE profile) => profile.Id.Where((ulong x) => x > 0UL).ToList<ulong>());
		...
		public async void RefreshDevice(reSENSI_CONTROLLER_PROFILE_EX profileEx)
		{
			foreach (object obj in Enum.GetValues(typeof(Slot)))
			{
				Slot slot = (Slot)obj;
				if (profileEx.ServiceProfileIds[slot] != 0 && profileEx.Enabled[slot] && reSENSI_CONTROLLER_PROFILE_Extensions.IsExternalDeviceWithBluetoothPresent(profileEx.Profiles[slot]) && profileEx.Profiles[slot].VirtualType == 48U && Convert.ToBoolean((uint)(profileEx.Profiles[slot].VirtualFlags & 64)))
				{
		...
		public async Task ExternalDeviceBluetoothReconnect(reSENSI_CONTROLLER_PROFILE_EX profileEx, Slot _slot)
		{
			if (profileEx.ServiceProfileIds[_slot] != 0 && profileEx.Enabled[_slot] && !profileEx.ProfilesState[_slot].RemoteConnected && profileEx.IsExternalBluetoothPresent())
			{
				...
				reSENSI_SET_PROFILE_INFO[] array = new reSENSI_SET_PROFILE_INFO[] { reSENSI_SET_PROFILE_INFO.CreateBlankInstance() };
				array[0].ProfileId = profileEx.ServiceProfileIds[_slot];
				reSENSI_SET_PROFILE_INFO[] array2 = array;
				int num = 0;
				array2[num].Flags = array2[num].Flags | 1U;
				reSENSI_SET_PROFILE_INFO[] array3 = array;
				int num2 = 0;
				array3[num2].Flags = array3[num2].Flags | 32U;
				reSENSI_SET_PROFILE_INFO[] array4 = array;
				int num3 = 0;
		...
		public async Task ExternalDeviceBluetoothReconnect(string gamepadId, Slot _slot)
		{
			if (this.GamepadService.GetRemapState(gamepadId) == 1)
			{
				...
				reSENSI_CONTROLLER_PROFILE_EX profileEx = this.GamepadService.GetProfileEx(gamepadId);
				if (profileEx != null)
				{
				...
				{
					Wrapper<reSENSI_CONTROLLER_PROFILE_EX> wrapper = this.GamepadService.ServiceProfilesCollection.FirstOrDefault((Wrapper<reSENSI_CONTROLLER_PROFILE_EX> sp) => sp.Value.SlotsWrapperId == slotsWrapperId);
					if (wrapper != null)
					{
		...
		public async Task ExternalDeviceDisableRemapForSerialPort(string serialPort)
		{
			if (!string.IsNullOrEmpty(serialPort))
			{
				...
				{
					Func<reSENSI_CONTROLLER_PROFILE, bool> <>9__0;
					foreach (Wrapper<reSENSI_CONTROLLER_PROFILE_EX> wrapper in serviceProfileWithExternalSerialPort)
					{
						IEnumerable<reSENSI_CONTROLLER_PROFILE> profiles = wrapper.Value.Profiles;
						Func<reSENSI_CONTROLLER_PROFILE, bool> func;
						if ((func = <>9__0) == null)
						{
							func = (<>9__0 = (reSENSI_CONTROLLER_PROFILE x) => reSENSI_CONTROLLER_PROFILE_Extensions.IsExternalDeviceWithSerialPortPresent(x) && reSENSI_CONTROLLER_PROFILE_Extensions.GetPortName(x) == serialPort.ToUpper());
						}
					...
					List<Wrapper<reSENSI_CONTROLLER_PROFILE_EX>>.Enumerator enumerator = default(List<Wrapper<reSENSI_CONTROLLER_PROFILE_EX>>.Enumerator);
				}
			}
		}
