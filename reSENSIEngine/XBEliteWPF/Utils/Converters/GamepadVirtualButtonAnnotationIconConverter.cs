	...
	public class GamepadVirtualButtonAnnotationIconConverter : MarkupExtension, IValueConverter, IMultiValueConverter
	{
		...
		public object Convert(object value, Type targetType, object parameter, CultureInfo culture)
		{
			...
			if (value is VirtualGamepadType)
			{
				...
				controllerTypeEnum = new ControllerTypeEnum?(VirtualControllerTypeExtensions.GetControllerType(virtualGamepadType));
			}
			if (value is ControllerTypeEnum)
			{
				ControllerTypeEnum controllerTypeEnum2 = (ControllerTypeEnum)value;
				controllerTypeEnum = new ControllerTypeEnum?(controllerTypeEnum2);
			}
			ControllerTypeEnum[] array = value as ControllerTypeEnum[];
			if (array != null)
			{
				controllerTypeEnum = new ControllerTypeEnum?(ControllerTypeExtensions.GetFirstOfFamily(array, 0));
			}
			if (value is GamepadButton)
			{
			...
			if (parameter is VirtualGamepadType)
			{
				...
				controllerTypeEnum = new ControllerTypeEnum?(VirtualControllerTypeExtensions.GetControllerType(virtualGamepadType2));
			}
			if (parameter is ControllerTypeEnum)
			{
				ControllerTypeEnum controllerTypeEnum3 = (ControllerTypeEnum)parameter;
				controllerTypeEnum = new ControllerTypeEnum?(controllerTypeEnum3);
			}
			ControllerTypeEnum[] array2 = parameter as ControllerTypeEnum[];
			if (array2 != null)
			{
				controllerTypeEnum = new ControllerTypeEnum?(ControllerTypeExtensions.GetFirstOfFamily(array2, 0));
			}
			...
			{
				return XBUtils.GetAnnotationDrawingForVirtualGamepadButton(gamepadButton.Value, null, null, controllerTypeEnum, null);
			}
			return null;
		}
