...
namespace XBEliteWPF.Utils.Extensions
{
	public static class CollectionExtensions
	{
		...
		public static reSENSI_CONTROLLER_PROFILE_EX FindByAmiiboUid(this IEnumerable<Wrapper<reSENSI_CONTROLLER_PROFILE_EX>> collection, byte[] amiiboUid)
		{
			Wrapper<reSENSI_CONTROLLER_PROFILE_EX> wrapper = collection.FindWrapperByAmiiboUid(amiiboUid);
			if (wrapper == null)
			{
				return null;
			}
		...
		public static Wrapper<reSENSI_CONTROLLER_PROFILE_EX> FindWrapperByAmiiboUid(this IEnumerable<Wrapper<reSENSI_CONTROLLER_PROFILE_EX>> collection, byte[] amiiboUid)
		{
			if (amiiboUid == null || amiiboUid.Length != 7)
			{
				return null;
			}
			Func<reSENSI_CONTROLLER_PROFILE_EX.AmiiboWrapper, bool> <>9__1;
			return collection.FirstOrDefault(delegate(Wrapper<reSENSI_CONTROLLER_PROFILE_EX> pex)
			{
				IEnumerable<reSENSI_CONTROLLER_PROFILE_EX.AmiiboWrapper> amiibo = pex.Value.Amiibo;
				Func<reSENSI_CONTROLLER_PROFILE_EX.AmiiboWrapper, bool> func;
				if ((func = <>9__1) == null)
				{
					func = (<>9__1 = (reSENSI_CONTROLLER_PROFILE_EX.AmiiboWrapper p) => p.IsUidEqual(amiiboUid));
				}
		...
		public static List<reSENSI_CONTROLLER_PROFILE> FindServiceProfilesByExternalMacAddress(this IEnumerable<Wrapper<reSENSI_CONTROLLER_PROFILE_EX>> collection, ulong macAddress)
		{
			List<reSENSI_CONTROLLER_PROFILE> list = new List<reSENSI_CONTROLLER_PROFILE>();
			if (macAddress == 0UL)
			{
				return list;
			}
			Func<reSENSI_CONTROLLER_PROFILE, bool> <>9__1;
			Wrapper<reSENSI_CONTROLLER_PROFILE_EX> wrapper = collection.FirstOrDefault(delegate(Wrapper<reSENSI_CONTROLLER_PROFILE_EX> pex)
			{
				IEnumerable<reSENSI_CONTROLLER_PROFILE> profiles2 = pex.Value.Profiles;
				Func<reSENSI_CONTROLLER_PROFILE, bool> func;
				if ((func = <>9__1) == null)
				{
					func = (<>9__1 = (reSENSI_CONTROLLER_PROFILE p) => p.RemoteBthAddr == macAddress);
				}
			...
			if (((wrapper != null) ? wrapper.Value : null) != null)
			{
				...
				{
					if (reSENSI_CONTROLLER_PROFILE.RemoteBthAddr == macAddress)
					{
						list.Add(reSENSI_CONTROLLER_PROFILE);
					}
				}
			}
			return list;
