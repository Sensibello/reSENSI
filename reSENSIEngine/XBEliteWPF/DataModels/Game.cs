...
using reSENSICommon.Infrastructure.JSONModel;
...
namespace XBEliteWPF.DataModels
{
	public class Game : IComparable
	{
		...
		{
			if (configSourcePath.EndsWith(".reSENSI"))
			{
		...
		{
			string text = Path.Combine(Engine.UserSettingsService.ConfigsFolderPath, this.Name + "\\Controller\\");
			Config config = new Config(text + XBUtils.NormalizeToMaxPathTrimFilename(text, configName.Trim(), ".reSENSI", null) + ".reSENSI", this._configFileService, this._gameProfilesService, this._gamepadServiceLazy, this._licensingService, this);
		...
		public ImportConfigResult ImportConfig(string configName, string configContent, bool isCloning, bool isRewrite)
		{
			...
			if (isCloning)
			{
				configPath = text + XBUtils.NormalizeToMaxPathTrimFilename(text, configName.Trim(), ".reSENSI", null) + ".reSENSI";
				int num = 0;
				...
				while (File.Exists(configPath))
				{
					...
					configPath = text + XBUtils.NormalizeToMaxPathTrimFilename(text, text2.Trim(), ".reSENSI", text4) + ".reSENSI";
					num++;
				}
			...
			if (File.Exists(configPath) && !isRewrite)
			{
				...
				{
					configPath = DSUtils.GetUniqueFileNameWithParentheses(directoryName, configName + ".reSENSI");
				}
			}
			try
			{
