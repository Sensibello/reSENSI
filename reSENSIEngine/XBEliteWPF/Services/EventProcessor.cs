...
namespace XBEliteWPF.Services
{
	public class EventProcessor : IEventProcessor
	{
		...
		public async void ProcessHiddenServiceCommand(ushort profileID, reSENSIHiddenServiceCommand reSENSIHiddenServiceCommand)
		{
			...
			if (reSENSIHiddenServiceCommand == 26 || reSENSIHiddenServiceCommand == 27 || reSENSIHiddenServiceCommand == 28)
			{
				if (reSENSIHiddenServiceCommand == 26)
				{
					Tracer.TraceWrite("ProcessHiddenServiceCommand: OverlayShowGamepad", false);
				}
				if (reSENSIHiddenServiceCommand == 27)
				{
					Tracer.TraceWrite("ProcessHiddenServiceCommand: OverlayShowMappings", false);
				}
				if (reSENSIHiddenServiceCommand == 28)
				{
				...
				Wrapper<reSENSI_CONTROLLER_PROFILE_EX> wrapper = this._gamepadService.ServiceProfilesCollection.FirstOrDefault((Wrapper<reSENSI_CONTROLLER_PROFILE_EX> sp) => sp.Value.SlotManagerProfileId == CS$<>8__locals1.profileID);
			...
			else
			{
				...
				Wrapper<reSENSI_CONTROLLER_PROFILE_EX> wrapper2 = this._gamepadService.ServiceProfilesCollection.FirstOrDefault((Wrapper<reSENSI_CONTROLLER_PROFILE_EX> sp) => sp.Value.ServiceProfileIds.Contains(CS$<>8__locals1.profileID));
			...
			if (CS$<>8__locals1.profile != null)
			{
				...
				if (reSENSIHiddenServiceCommand.IsLEDReactionRequired())
				{
					this.ProcessLEDReactionRequiredCommand(CS$<>8__locals1.profile, reSENSIHiddenServiceCommand);
				}
				...
				else
				{
					...
					if (controller != null)
					{
						...
						Slot slot2 = reSENSI_CONTROLLER_PROFILE_Extensions.GetSlot(CS$<>8__locals1.profile.Value);
						GamepadProfile gamepadProfile2 = gamepadProfiles.SlotProfiles.TryGetValue(slot2);
						if ((reSENSIHiddenServiceCommand >= 0 && reSENSIHiddenServiceCommand <= 21) || (reSENSIHiddenServiceCommand >= 30 && reSENSIHiddenServiceCommand <= 50))
						{
							bool flag2 = reSENSIHiddenServiceCommand >= 30 && reSENSIHiddenServiceCommand <= 50;
							string shiftName = this.GetShiftName(gamepadProfiles, controller, reSENSIHiddenServiceCommand);
							HiddenCommandEventHandler overlayShiftChange = this.OverlayShiftChange;
							if (overlayShiftChange != null)
							{
								overlayShiftChange(reSENSIHiddenServiceCommand, gamepadProfile2, shiftName, controller, flag2);
							}
						}
						switch (reSENSIHiddenServiceCommand)
						{
						case 22:
							slot = new Slot?(0);
							break;
						...
						case 28:
						case 29:
							slot = new Slot?(reSENSI_CONTROLLER_PROFILE_Extensions.GetSlot(CS$<>8__locals1.profile.Value));
							flag = reSENSIHiddenServiceCommand == 28;
							break;
						}
						if (slot == null)
						{
		...
		private string GetShiftName(GamepadProfiles gamepadProfiles, BaseControllerVM controller, reSENSIHiddenServiceCommand command)
		{
			string text = null;
			Slot currentSlot = controller.CurrentSlot;
		...
		private async void ProcessLEDReactionRequiredCommand(reSENSI_CONTROLLER_PROFILE? profile, reSENSIHiddenServiceCommand reSENSIHiddenServiceCommand)
		{
			...
			defaultInterpolatedStringHandler.AppendFormatted<reSENSIHiddenServiceCommand>(reSENSIHiddenServiceCommand);
			...
			if (profile != null && Engine.UserSettingsService.IsLedSettingsEnabled)
			{
				...
				else
				{
					...
					else
					{
						...
						int num = 0;
						if (reSENSIHiddenServiceCommand >= 1 && reSENSIHiddenServiceCommand <= 21)
						{
							num = (int)reSENSIHiddenServiceCommand;
						}
						if (reSENSIHiddenServiceCommand >= 30 && reSENSIHiddenServiceCommand <= 50)
						{
							num = (int)(reSENSIHiddenServiceCommand - 30 + 1);
						}
						if (reSENSIHiddenServiceCommand != null)
						{
							switch (reSENSIHiddenServiceCommand)
							{
							case 22:
