...
namespace XBEliteWPF.Services.HttpServer
{
	...
	public class GamepadServiceController : ControllerBase
	{
		...
		{
			ServiceResponseWrapper<reSENSI_GET_PROFILE_STATE_RESPONSE> serviceResponseWrapper = await Engine.XBServiceCommunicator.GetProfileState(profileId, false);
		...
		{
			List<reSENSI_CONTROLLER_PROFILE> list = Engine.GamepadService.ServiceProfilesCollection.FindServiceProfilesByExternalMacAddress(macAddress);
			foreach (reSENSI_CONTROLLER_PROFILE reSENSI_CONTROLLER_PROFILE in list)
			{
				string text = XBUtils.CalculateID(reSENSI_CONTROLLER_PROFILE.Id);
				await Engine.GamepadService.RestoreDefaults(text, new List<Slot> { reSENSI_CONTROLLER_PROFILE_Extensions.GetSlot(reSENSI_CONTROLLER_PROFILE) });
			}
			List<reSENSI_CONTROLLER_PROFILE>.Enumerator enumerator = default(List<reSENSI_CONTROLLER_PROFILE>.Enumerator);
			return this.Ok();
		}
		...
		public IActionResult GetAppliedConfig([FromBody] SelectSlotInfo slotInfo)
		{
			...
			if (gamepadProfiles != null)
			{
				...
				{
					reSENSI_CONTROLLER_PROFILE_EX reSENSI_CONTROLLER_PROFILE_EX = Engine.GamepadService.ServiceProfilesCollection.FindByID(slotInfo.ID);
					if (reSENSI_CONTROLLER_PROFILE_EX != null && reSENSI_CONTROLLER_PROFILE_Extensions.IsSlotWithUserConfig(reSENSI_CONTROLLER_PROFILE_EX.Profiles[slotInfo.Slot]))
					{
		...
		public async Task<IActionResult> ApplyAmiibo([FromBody] ApplyAmiiboInfo amiiboInfo)
		{
			...
			else
			{
				...
				{
					reSENSI_CONTROLLER_PROFILE_EX pEx = Engine.GamepadService.ServiceProfilesCollection.FindByID(amiiboInfo.GamepadId);
					if (pEx.Profiles[amiiboInfo.Slot].VirtualType == 48U && pEx.Enabled[amiiboInfo.Slot])
					{
						...
						ServiceResponseWrapper<reSENSI_SET_PROFILE_STATE_RESPONSE> serviceResponseWrapper = await Engine.XBServiceCommunicator.SetProfileState(array, false);
				...
				if (UtilsCommon.IsEnginePeripheralsShouldBeComposited(CS$<>8__locals1.info.Id, CS$<>8__locals1.info.EngineControllerTypes, ref list))
				{
					if (list.Count > 1)
					{
						...
						foreach (Tuple<ulong, ControllerTypeEnum> tuple in list)
						{
							...
							reSENSI_CONTROLLER_INFO.Id = tuple.Item1;
							reSENSI_CONTROLLER_INFO.Type = ControllerTypeExtensions.ConvertEnumToPhysicalType(tuple.Item2);
							reSENSI_CONTROLLER_INFO.ContainerId = guid;
							ControllerVM controllerVM = new ControllerVM(reSENSI_CONTROLLER_INFO, false);
							if (flag)
							{
