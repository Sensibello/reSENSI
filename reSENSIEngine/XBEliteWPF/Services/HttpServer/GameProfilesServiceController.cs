...
using reSENSICommon.Infrastructure.RadialMenu.Utils;
	...
	public class GameProfilesServiceController : ControllerBase
	{
		...
		public IActionResult CopyDirectory([FromBody] CopyDirectoryParams copyParams)
		{
			try
			{
				...
				{
					string text2 = XBUtils.NormalizeToMaxPathTrimFilename(copyParams.DestinationDir, Path.GetFileNameWithoutExtension(fileInfo.Name).Trim(), ".reSENSI", null) + ".reSENSI";
		...
		public IActionResult RenameConfig([FromBody] RenameConfigParams configParams)
		{
			...
			try
			{
				...
				string text = directoryName + "\\" + fileNameWithoutExtension + ".reSENSI.bak";
				string text2 = directoryName + "\\" + fileNameWithoutExtension + ".reSENSI";
				List<string> list = new List<string>();
				list.Add(".reSENSI");
				list.Add(".reSENSI.bak");
				...
				{
					System.IO.File.Move(text, directoryName + "\\" + config.Name + ".reSENSI.bak");
					System.IO.File.SetAttributes(directoryName + "\\" + config.Name + ".reSENSI.bak", FileAttributes.Normal);
				}
				if (System.IO.File.Exists(text2))
				{
					string text4 = directoryName + "\\" + config.Name + ".reSENSI";
		...
		public IActionResult SavePreset([FromBody] SaveConfigParams<ConfigData> configParams)
		{
			...
			new Config(Path.Combine(presetsFolderPath, text2) + ".reSENSI", Engine.ConfigFileService, Engine.GameProfilesService, Engine.GamepadServiceLazy, Engine.LicensingService, null).SaveConfig(configParams.ConfigData, configParams.ClientID);
			IGameProfilesService gameProfilesService2 = Engine.GameProfilesService;
