...
using DiscSoftreSENSIGuiNamespace;
using DTOverlay;
...
namespace XBEliteWPF.Services
{
	public class WindowsMessageProcessor : IWindowsMessageProcessor
	{
		...
		private IntPtr WndProc(IntPtr hwnd, int msg, IntPtr wParam, IntPtr lParam, ref bool handled)
		{
			...
			if (msg == 537)
			{
				if (wParam.ToInt32() == 32774)
				{
					...
					if (dev_BROADCAST_HDR.dbch_devicetype == 6 && dev_BROADCAST_HDR.dbch_size >= Marshal.SizeOf(typeof(UtilsNative.Win32.DEV_BROADCAST_HANDLE)))
					{
						...
						if (dev_BROADCAST_HANDLE.dbch_eventguid.Equals(DiscSoftreSENSIGui.GUID_reSENSI_SERVICE_CONTROLLERS_CHANGED))
						{
							this.ProcessEvent("DiscSoftreSENSI_msg_1", intPtr2, intPtr);
						}
						if (dev_BROADCAST_HANDLE.dbch_eventguid.Equals(DiscSoftreSENSIGui.GUID_reSENSI_SERVICE_PROFILES_CHANGED))
						{
							this.ProcessEvent("DiscSoftreSENSI_msg_3", intPtr2, intPtr);
						}
						if (dev_BROADCAST_HANDLE.dbch_eventguid.Equals(DiscSoftreSENSIGui.GUID_reSENSI_SERVICE_PROFILE_STATE_CHANGED))
						{
							this.ProcessEvent("DiscSoftreSENSI_msg_17", intPtr2, intPtr);
						}
						if (dev_BROADCAST_HANDLE.dbch_eventguid.Equals(DiscSoftreSENSIGui.GUID_reSENSI_SERVICE_PHYS_SLOT_1_SELECTED))
						{
							this.ProcessEvent("DiscSoftreSENSI_msg_phys_slot_1", intPtr2, intPtr);
						}
						if (dev_BROADCAST_HANDLE.dbch_eventguid.Equals(DiscSoftreSENSIGui.GUID_reSENSI_SERVICE_PHYS_SLOT_2_SELECTED))
						{
							this.ProcessEvent("DiscSoftreSENSI_msg_phys_slot_2", intPtr2, intPtr);
						}
						if (dev_BROADCAST_HANDLE.dbch_eventguid.Equals(DiscSoftreSENSIGui.GUID_reSENSI_SERVICE_PHYS_SLOT_3_SELECTED))
						{
							this.ProcessEvent("Disc SoftreSENSI_msg_phys_slot_3", intPtr2, intPtr);
						}
						if (dev_BROADCAST_HANDLE.dbch_eventguid.Equals(DiscSoftreSENSIGui.GUID_reSENSI_SERVICE_PHYS_SLOT_4_SELECTED))
						{
							this.ProcessEvent("DiscSoftreSENSI_msg_phys_slot_4", intPtr2, intPtr);
						}
						if (dev_BROADCAST_HANDLE.dbch_eventguid.Equals(DiscSoftreSENSIGui.GUID_reSENSI_SERVICE_MACRO_BROADCAST))
						{
							this.ProcessEvent("DiscSoftreSENSI_msg_macro_command", intPtr2, intPtr);
						}
						if (dev_BROADCAST_HANDLE.dbch_eventguid.Equals(DiscSoftreSENSIGui.GUID_reSENSI_SERVICE_GYRO_CALIBRATION_FINISHED))
						{
							this.ProcessEvent("DiscSoftreSENSI_msg_18", intPtr2, intPtr);
						}
						if (dev_BROADCAST_HANDLE.dbch_eventguid.Equals(DiscSoftreSENSIGui.GUID_reSENSI_SERVICE_AMIIBO_UNLOAD_BY_UID))
						{
							this.ProcessEvent("DiscSoftreSENSI_msg_amiibo_unload_uid", intPtr2, intPtr);
						}
						if (dev_BROADCAST_HANDLE.dbch_eventguid.Equals(DiscSoftreSENSIGui.GUID_reSENSI_SERVICE_HONEYPOT_PAIRING_REJECTED))
						{
							this.ProcessEvent("DiscSoftreSENSI_msg_honeypot_pairing_rejected", intPtr2, intPtr);
						}
					}
				}
			...
			{
				WindowMessageEvent windowMessageEvent = new WindowMessageEvent("DiscSoftreSENSI_msg_15", wParam, lParam);
			...
			if ((long)msg == (long)((ulong)this.CompositeDevicesChanged))
			{
				...
				WindowMessageEvent windowMessageEvent2 = new WindowMessageEvent("DiscSoftreSENSI_msg_12", wParam, lParam);
		...
		private void ProcessEvent(string msg, IntPtr wParam, IntPtr lParam)
		{
			...
			if (msg == "DiscSoftreSENSI_msg_9")
			{
				Tracer.TraceWrite("Before processing wnd message reSENSI_MESSAGE_REMAP_ON", false);
				this._eventAggregator.GetEvent<RemapOn>().Publish(new ulong?(DSUtils.GetUlongFromWMPayload(windowMessageEvent)));
			}
			if (msg == "DiscSoftreSENSI_msg_16")
			{
				Tracer.TraceWrite("Before processing wnd message reSENSI_MESSAGE_REMAP_OFF", false);
				this._eventAggregator.GetEvent<RemapOff>().Publish(new ulong?(DSUtils.GetUlongFromWMPayload(windowMessageEvent)));
			}
			if (msg == "DiscSoftreSENSI_msg_1")
			{
				Tracer.TraceWrite("Before processing wnd message reSENSI_MESSAGE_CONTROLLER_CHANGED", false);
				this._eventAggregator.GetEvent<GamepadChanged>().Publish(windowMessageEvent);
			}
			if (msg == "DiscSoftreSENSI_msg_3")
			{
				Tracer.TraceWrite("Before processing wnd message reSENSI_MESSAGE_SERVICE_PROFILES_CHANGED", false);
				this._eventAggregator.GetEvent<ServiceProfilesChanged>().Publish(windowMessageEvent);
			}
			if (msg == "DiscSoftreSENSI_msg_17")
			{
				Tracer.TraceWrite("Before processing wnd message reSENSI_MESSAGE_PROFILE_STATE_CHANGED", false);
				this._eventAggregator.GetEvent<ServiceProfileStateChanged>().Publish(windowMessageEvent);
			}
			if (msg == "DiscSoftreSENSI_msg_phys_slot_1")
			{
				Tracer.TraceWrite("Before processing wnd message reSENSI_MESSAGE_PHYS_SLOT_1_SELECTED", false);
				this._eventAggregator.GetEvent<PhysSlot1Selected>().Publish(windowMessageEvent);
			}
			if (msg == "DiscSoftreSENSI_msg_phys_slot_2")
			{
				Tracer.TraceWrite("Before processing wnd message reSENSI_MESSAGE_PHYS_SLOT_2_SELECTED", false);
				this._eventAggregator.GetEvent<PhysSlot2Selected>().Publish(windowMessageEvent);
			}
			if (msg == "Disc SoftreSENSI_msg_phys_slot_3")
			{
				Tracer.TraceWrite("Before processing wnd message reSENSI_MESSAGE_PHYS_SLOT_3_SELECTED", false);
				this._eventAggregator.GetEvent<PhysSlot3Selected>().Publish(windowMessageEvent);
			}
			if (msg == "DiscSoftreSENSI_msg_phys_slot_4")
			{
				Tracer.TraceWrite("Before processing wnd message reSENSI_MESSAGE_PHYS_SLOT_4_SELECTED", false);
				this._eventAggregator.GetEvent<PhysSlot4Selected>().Publish(windowMessageEvent);
			}
			if (msg == "DiscSoftreSENSI_msg_macro_command")
			{
				Tracer.TraceWrite("Before processing wnd message reSENSI_MESSAGE_MACRO_COMMAND", false);
				this._eventAggregator.GetEvent<MacroCommandBroadcast>().Publish(windowMessageEvent);
			}
			if (msg == "DiscSoftreSENSI_msg_18")
			{
				Tracer.TraceWrite("Before processing wnd message reSENSI_MESSAGE_GYRO_CALIBRATION_FINISHED", false);
				this._eventAggregator.GetEvent<GyroCalibrationFinished>().Publish(windowMessageEvent);
			}
			if (msg == "DiscSoftreSENSI_msg_amiibo_unload_uid")
			{
				Tracer.TraceWrite("Before processing wnd message reSENSI_MESSAGE_AMIIBO_UNLOAD_BY_UID", false);
				this._eventAggregator.GetEvent<AmiiboUnloadByUidBroadcast>().Publish(windowMessageEvent);
			}
			if (msg == "DiscSoftreSENSI_msg_honeypot_pairing_rejected")
			{
				Tracer.TraceWrite("Before processing wnd message reSENSI_MESSAGE_HONEYPOT_PAIRING_REJECTED", false);
		...
		private uint CompositeDevicesChanged = WindowsMessageProcessor.RegisterWindowMessage("DiscSoftreSENSI_msg_12");

		private uint InitializedPeripheralDevicesChanged = WindowsMessageProcessor.RegisterWindowMessage("DiscSoftreSENSI_msg_15");

		private uint RemapOn = WindowsMessageProcessor.RegisterWindowMessage("DiscSoftreSENSI_msg_9");

		private uint RemapOff = WindowsMessageProcessor.RegisterWindowMessage("DiscSoftreSENSI_msg_16");
