...
using reSENSICommon.Network.HTTP.DataTransferObjects.Events.Emulator;
...
namespace reSENSIEngine.Services.HttpServer
{
	...
	public class EmulatorEventController : ControllerBase
	{
		...
		private void OnSlotChanged(BaseControllerVM controller, GamepadProfile gamepadProfile, Slot slot, reSENSI_CONTROLLER_PROFILE? profile, bool physical)
		{
			if (!this.IsThisEngineController(controller))
			{
				return;
		...
		private void OnConfigApplied(BaseControllerVM controller, GamepadProfile gamepadProfile, Slot slot, reSENSI_CONTROLLER_PROFILE? profile)
		{
			if (!this.IsThisEngineController(controller))
			{
				return;
		...
		private async void MonitorVibration(ulong controllerId)
		{
			...
			for (;;)
			{
				...
				{
					await Task.Delay(50);
					reSENSI_CONTROLLER_PROFILE_EX reSENSI_CONTROLLER_PROFILE_EX = Engine.GamepadService.ServiceProfilesCollection.FindByControllerID(controllerId);
					if (reSENSI_CONTROLLER_PROFILE_EX == null)
					{
						continue;
					}
					Slot currentSlot = reSENSI_CONTROLLER_PROFILE_EX.GetCurrentSlot();
					ushort num = reSENSI_CONTROLLER_PROFILE_EX.ServiceProfileIds[currentSlot];
