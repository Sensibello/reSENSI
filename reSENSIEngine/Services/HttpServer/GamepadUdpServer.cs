...
using reSENSICommon.Utils.Converters;
using reSENSICommon.Utils.Crc32;
...
namespace reSENSIEngine.Services.HttpServer
{
	public class GamepadUdpServer
	{
		...
		{
			reSENSI_CONTROLLER_PROFILE_EX profileExByServiceProfileId = Engine.GamepadService.GetProfileExByServiceProfileId((ulong)serviceProfileID);
			if (profileExByServiceProfileId != null)
			{
				reSENSI_CONTROLLER_PROFILE? reSENSI_CONTROLLER_PROFILE = profileExByServiceProfileId.FindProfileById(serviceProfileID);
				if (reSENSI_CONTROLLER_PROFILE != null)
				{
					return reSENSI_CONTROLLER_PROFILE.Value.Id;
				}
			}
			return null;
		}
